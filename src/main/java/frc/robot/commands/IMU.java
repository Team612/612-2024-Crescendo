...
public class IMU extends TimedRobot{
  // Tread variables
  private java.util.Timer executor;

  public void periodiccaller() {
    addPeriodic(() -> {
      new BNO055UpdateTask(this);
    }, 0.01, 0.005);
  }
   ...
   */
  public IMU(I2C.Port port, byte address) {
   ...
   */
  public static IMU getInstance(opmode_t mode, vector_type_t vectorType,
      I2C.Port port, byte address) {
    if (instance == null) {
      instance = new IMU(port, address);
    }
    requestedMode = mode;
  ...
  private class BNO055UpdateTask extends TimerTask {
    ...
    private BNO055UpdateTask(IMU imu) {
      if (imu == null) {
