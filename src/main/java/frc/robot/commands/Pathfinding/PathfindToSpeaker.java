...
public class PathfindToSpeaker extends Command {
  ...
  public PathfindToSpeaker(Drivetrain d, PoseEstimator p, Vision v) {
  ...
  public void initialize() {
    if(DriverStation.getAlliance().get() == DriverStation.Alliance.Blue) {
      targetPose = new Pose2d(
        m_vision.return_tag_pose(7).getX() + 1,
        m_vision.return_tag_pose(7).getY(),
        new Rotation2d(Units.degreesToRadians(180))
      );
    } else {
      targetPose = new Pose2d(
        m_vision.return_tag_pose(4).getX() - 1,
        m_vision.return_tag_pose(4).getY(),
