...
public class AutoShootStartDouble extends Command {
  ...
  public AutoShootStartDouble(Shooter s, Intake i) {
    m_shooter = s;
    m_intake = i;
    addRequirements(s, i);
  ...
  public void execute() {
    ...
    // if(m_shooter.getCurrent() < 16 && spikeDone) {
      if(time.get() >= 2.5) {
        ...
        m_shooter.shoot(Constants.ShooterConstants.shooterLeftSpeedSpeaker * 2, Constants.ShooterConstants.shooterRightSpeedSpeaker * 2);
      }
    // } else {
    //   m_shooter.shoot(Constants.ShooterConstants.shooterLeftSpeedSpeaker, Constants.ShooterConstants.shooterRightSpeedSpeaker);
    // }
